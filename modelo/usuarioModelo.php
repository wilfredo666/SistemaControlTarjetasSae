...
class ModeloUsuario {
    ...
    static public function mdlEditUsuario($data){
        ...
        $stmt = Conexion::conectar()->prepare( "update usuario set nombre_usuario='$nomUsuario', pass_usuario='$passUsuario',rol_usuario='$rolUsuario' ,foto_usuario='$imgUsuario' ,estado_usuario='$estadoUsuario' where id_usuario=$idUsuario");
...
$stmt = Conexion::conectar()->prepare("delete from usuario where id_usuario=$data and estado_usuario=0");
            $stmt->execute();
        $t=$stmt->rowCount();
        if($t>0){
            return "correcto";
